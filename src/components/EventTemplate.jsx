...
const EventTemplate = ({ eventData, textPos, lastEvent = false }) => {
  ...
  return (
    <article className={`flex flex-col ${textPos === "left" ? `lg:mr-20` : `lg:ml-10`} items-start space-y-5 lg:h-[765px] ${lastEvent === true ? `overflow-hidden pt-20 -mt-20` : ``}`}>
      <div className={`lg:relative lg:bottom-155 lg:order-2 self-stretch z-30 mt-24 lg:mt-0 lg:self-start ${textPos === "left" ? `lg:left-10` : `lg:left-106`}`}>
        <ClientOnlyWrapper>
          <SlideElementWrapper slideFrom={headingSlideFrom} duration="1.1">
      ...
      <div className={`lg:relative lg:bottom-254 lg:order-3 z-40 -mt-3 lg:-mt-0 ${textPos === "left" ? `lg:left-23` : `lg:left-119`}`}>
        <ClientOnlyWrapper>
          <SlideElementWrapper slideFrom={textSlideFrom} duration="1.1">
